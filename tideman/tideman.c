...
int main(int argc, string argv[])
{
    ...
    printf("pair_count: %i", pair_count);
    printf("\n");
...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count - 1; i++)
    {
        ...
        {
            if (get_victory_strength(j) > get_victory_strength(greatest_index))
            {
                greatest_index = j;
            }
        }
        ...
        if (greatest_index != i)
        {
            pair greatest_pair = pairs[greatest_index];

            pairs[greatest_index] = pairs[i];

            pairs[i] = greatest_pair;
        }
    }
    return;
}
